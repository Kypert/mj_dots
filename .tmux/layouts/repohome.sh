...
# Argument 2 = Repo name (assume ~/proj/<repo>)
...
[[ $1 != -1 ]] && tmux $TMUX_ADDITIONAL_OPTS new-window -n $1 -c ~/proj/$2
...
tmux $TMUX_ADDITIONAL_OPTS send-keys -t 1 "cd ~/proj/$2" C-j
tmux $TMUX_ADDITIONAL_OPTS send-keys -t 2 "cd ~/proj/$2" C-j
